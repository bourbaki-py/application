...
class InstanceLoggerNamingConvention(Enum):
    ...
    keyword = 'use the keyword arg %s to pass in a custom instance logger name via the constructor' % LOG_NAME_KWARG
...
class Logged:
    ...
    def __logname__(self):
        ...
        if name is None:
            ...
            if convention is None:
                ...
                raise TypeError('__instance_naming__ attribute must be an instance of {} or a callable; got {}'
                                .format(InstanceLoggerNamingConvention, type(convention)))
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if convention is None:
            ...
            if convention is None:
                raise ValueError("__instance_naming__ attribute, if a string, must be one of {}"
                                 .format([c.name for c in InstanceLoggerNamingConvention]))
        elif not callable(convention) and not isinstance(convention, InstanceLoggerNamingConvention):
            raise TypeError("__instance_naming__ attribute must be either a callable taking a single instance of "
    ...
    def logger(obj, logger):
        if not isinstance(logger, Logger):
            raise TypeError("class logger must be an instance of logging.Logger; got {}"
                            .format(type(logger)))
        obj.__dict__['__logger__'] = logger

    def __getstate__(self):
        self.logger.debug("Getting state for pickling/copying, excluding __logger__ attribute")
    ...
    def __init__(self: Logged, *args, **kwargs):
        ...
        if self.__verbose__:
            ...
            try:
                ...
                logger.error("Initialization failed for %s instance with args %s",
                             cls, bound_args, exc_info=self.__log_tracebacks__)
        else:
            init(self, *args, **kwargs)
