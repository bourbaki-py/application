...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "x = [i for i in range(10)]; y = sum(x)",
            dict(x=list(range(10)), y=sum(range(10))),
        ),
        (
            "foobar=frozenset(zip(range(3), 'foo'))",
            dict(foobar=frozenset(zip(range(3), "foo"))),
        ),
...
def test_load_python_raises(source, exception):
    ...
    with pytest.raises(exception):
        _ = load_python(f)
    if not issubclass(
        exception, (PythonSourceConfigSyntaxError, PythonSourceConfigRuntimeError)
    ):
        with pytest.raises(exception):
