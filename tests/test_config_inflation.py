...
from bourbaki.application.typed_io.config.inflation import instance_from, ConfigTypedInputError, ConfigInflationError
...
@pytest.mark.parametrize('config_val,test_val,target_type', [
    # basic with positional args
    (dict(__classpath__=CLASSPATH, __args__=('1', 2)), Class('1', 2), Class),
    # with extra *args of correct type
    (dict(__classpath__=CLASSPATH, __args__=('1', 2, 'three')), Class('1', 2), Class),
    # with **kwargs
    (dict(__classpath__=CLASSPATH, __kwargs__=dict(x='1', y=2)), Class('1', 2), Class),
    # with extra **kwargs of correct tpye
    (dict(__classpath__=CLASSPATH, __kwargs__=dict(x='1', y=2, z=1.23)), Class('1', 2), Class),
    # parameterized
    (dict(__classpath__=CLASSPATH + '[int]', __args__=('1', 2)), Class('1', 2), Class[int]),
    (dict(__classpath__=CLASSPATH + '[str]', __args__=('1', '2')), Class('1', '2'), Class[str]),
    # config-specified is subtype of target type
    (dict(
        __classpath__=SUBCLASSPATH + '[bool]',
        __kwargs__=dict(x='1', y=True)
     ),
     Subclass('1', True),
     Class[int],
    ),
...
@pytest.mark.parametrize('config_val,target_type,exc_type,match', [
    # classpath to non-type
    (dict(__classpath__=MODULE + '.CLASSPATH'), Class, TypeError, 'not a type'),
    # classpath to non-subtype
    (dict(__classpath__='int'), Class, TypeError, r'not\b[\s\w]* a generic subclass of .*\b' + CLASSPATH),
    # classpath to non-subtype with params
    (dict(__classpath__=CLASSPATH + '[str]'), Class[int], TypeError, r'not\b[\s\w]* a generic subclass of .*\b' + CLASSPATH + '\[int\]'),
    # classpath is fine but parameterized arg is wrong type
    (dict(__classpath__=CLASSPATH + '[str]', __args__=('1', 2)), Class[str], ConfigInflationError, r'occurred for arg[\w]* y\b'),
    # classpath is fine and parameterized arg is fine but non-parameterized arg is wrong type
    (dict(__classpath__=CLASSPATH + '[str]', __args__=(1, '2')), Class[str], ConfigInflationError, r'occurred for arg[\w]* x\b'),
