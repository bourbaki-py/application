...
from typing import Mapping, List, MutableSet, Tuple, Union, Pattern, Counter, ByteString, ChainMap
...
@pytest.mark.parametrize(
    ...
    [(int, 1, 1, same_value),
     ...
     (Counter[str], {"foo": 1, "bar": 2}, cl.Counter(["foo", "bar", "bar"]), same_keyvals),
     (Counter[int], [("1", "1"), ("2", "2"), ("3", "3")], cl.Counter({1: 1, 2: 2, 3: 3}), same_keyvals),
     ...
     (MutableSet[bytes], ["b'foo'", [1, 2, 3]], {b'foo', b'\x01\x02\x03'}, same_contents),
     (Tuple[float, date, List[Path]], [1, "2018-01-01", ("foo", "bar")],
        (1.0, date(2018, 1, 1), [Path("foo"), Path("bar")]), same_contents),
     (Pattern[bytes], "foobar", re.compile(b"foobar"), same_value),
     (List[Tuple[float, bool]], ([1, True], [2, False]), [(1.0, True), (2.0, False)], same_contents),
     (Union[List[Tuple[date, tuple]], Mapping[date, tuple]],
      [["2018-01-01", [1, True]], ["2019-01-01", [False, 2]]],
      [(date(2018, 1, 1), (1, True)), (date(2019, 1, 1), (False, 2))], same_contents),
     (Union[Mapping[date, tuple], List[Tuple[date, ...]]],
      [["2018-01-01", [1, True]], ["2019-01-01", [False, 2]]],
      dict([(date(2018, 1, 1), (1, True)), (date(2019, 1, 1), (False, 2))]), same_contents),
     (Tuple[complex, ...], ["1+2j", 3+4j], (1+2j, 3+4j), same_contents),
     (ChainMap[int, range], [{1: "1:2", 2: [3, 4, 5]}, {"3": "6:7"}],
      cl.ChainMap({1: range(1, 2), 2: range(3, 4, 5), 3: range(6, 7)}), same_keyvals),
     (ChainMap[int, range], {1: "1:2", 2: [3, 4, 5], "3": "6:7"},
      cl.ChainMap({1: range(1, 2), 2: range(3, 4, 5), 3: range(6, 7)}), same_keyvals),
     ])
