...
class _TestCase:
    def __init__(self, type_,
                 cli_nargs=undefined, cli_repr=undefined, config_repr=undefined,
                 test_val=undefined, cli_test_val=undefined, config_test_val=undefined,
                 encoded_eq=equal_types, multi_test=False):
    ...
    def should_test(self, attr):
        attr_ = (dict(cli_parser="cli_test_val", config_decoder="config_test_val", config_encoder="test_val")
                 .get(attr, attr))
    ...
    def test_cli_parser(self):
        self._test_callable_attr("cli_parser", self.test_val, self.cli_test_val, CLIIOUndefined)

    def test_config_decoder(self):
        self._test_callable_attr("config_decoder", self.test_val, self.config_test_val, ConfigIOUndefined)

    def test_config_encoder(self):
        target_val = self.config_test_val[0] if self.multi_test else self.config_test_val
        test_val = [self.test_val] if self.multi_test else self.test_val
        self._test_callable_attr("config_encoder", target_val, test_val, ConfigIOUndefined, encoded=True)
...
datetime_tup = [datetime_.year, datetime_.month, datetime_.day, datetime_.hour, datetime_.minute,
                datetime_.second, datetime_.microsecond]
...
map_int_tup_date_ip = {0: (date_, ipv4), -1234567: (date_ - datetime.timedelta(days=100000), ipv4)}
map_int_or_date_tup_any_ip_ = {-1234567: (ipv4, ipv6), date_: (ipv6, ipv6),
                            date_ - datetime.timedelta(days=100000): (ipv4, ipv4, ipv6, ipv6)}
map_int_or_date_tup_any_ip_config = {
    str(k) if isinstance(k, int) else k.isoformat(): list(map(str, v)) for k, v in map_int_or_date_tup_any_ip_.items()
}
...
bool_float_or_str_config_repr = " OR ".join([bool_config_repr, type_spec(float), type_spec(str)])
...
class ConfigReprMeta(type):
    ...
    def config_repr(self):
        repr_ = {
            CLASSPATH_KEY: self.__classpath__,
            KWARGS_KEY: self.__kwargs__
        }
...
class SimpleConfigRepr(SimpleEQ, metaclass=ConfigReprMeta):
    def config_encode(self, *args):
        ...
        ns.pop("args", None)
        repr_ = {
            CLASSPATH_KEY: classpath(type(self)),
            KWARGS_KEY: ns,
        }
...
class custom_generic_class(SimpleConfigRepr, Generic[T_co], metaclass=SimpleConfigReprGenericMeta):
    ...
    def config_encode(self, t=None):
        ...
        if t is None:
            ...
            conf[CLASSPATH_KEY] = conf[CLASSPATH_KEY] + "[{}]".format(parameterized_classpath(t))

        conf["__kwargs__"]["xs"] = {str(k): enc(v) for k, v in conf["__kwargs__"]["xs"].items()}
...
if NEW_TYPING:
    ...
    for t, attr in itertools.product((str, datetime.date), ("__kwargs__", "__classpath__", "__sig_args__", "config_repr",)):
...
test_cases = [
    ...
    _TestCase(datetime.date, None, date_repr, date_repr, date_, [date_str], [date_str, date_int, date_tup],
              multi_test=True),
    _TestCase(datetime.datetime, None, datetime_repr, datetime_repr, datetime_, datetime_str, datetime_str),
    ...
    # Unions
    _TestCase(any_ip, None, "{}|{}".format(ipv4_repr, ipv6_repr), any_ip_config_repr,
              ipv4, ipv4_str, ipv4_str),
    _TestCase(any_ip, None, "{}|{}".format(ipv4_repr, ipv6_repr), any_ip_config_repr,
              ipv6, ipv6_str, ipv6_str),
    _TestCase(bool_float_or_str, None, bool_float_or_str_cli_repr, bool_float_or_str_config_repr,
              "asdf", "asdf", "asdf"),
    _TestCase(bool_float_or_str, None, bool_float_or_str_cli_repr, bool_float_or_str_config_repr,
              3.14, ".314e1", 3.14),
    _TestCase(bool_float_or_str, None, bool_float_or_str_cli_repr, bool_float_or_str_config_repr,
              False, "FALSE", False),
    # collections
    _TestCase(bytes, ZERO_OR_MORE, bytes_cli_repr, bytes_config_repr, bytes_, bytes_str, bytes_int),
    _TestCase(bytearray, ZERO_OR_MORE, bytes_cli_repr, bytes_config_repr, bytearray_, bytes_str, bytes_int),
    _TestCase(tuple, ZERO_OR_MORE, any_coll_cli_repr, any_coll_config_repr, tuple_str_, list_str, list_str),
    _TestCase(set, ZERO_OR_MORE, any_coll_cli_repr, any_coll_config_repr, set_str, list_str, list_str,
              equal_contents),
    _TestCase(frozenset, ZERO_OR_MORE, any_coll_cli_repr, any_coll_config_repr, frozenset_str, list_str, list_str,
              equal_contents),
    _TestCase(list, ZERO_OR_MORE, any_coll_cli_repr, any_coll_config_repr, list_str, list_str, list_str),
    # mappings
    _TestCase(Mapping[int, Tuple[datetime.date, ipaddress.IPv4Address]], undefined,
              undefined, {type_spec(int): [date_repr, ipv4_repr], ellipsis_: ellipsis_},
              test_val=map_int_tup_date_ip,
              cli_test_val=undefined,
              config_test_val={str(k): [d.isoformat(), str(i)] for k, (d, i) in map_int_tup_date_ip.items()}),
    _TestCase(Mapping[Union[int, datetime.date], Tuple[Union[ipaddress.IPv4Address, ipaddress.IPv6Address], ...]],
              undefined, undefined,
              config_repr={"{} OR {}".format(type_spec(int), date_repr): [any_ip_config_repr, ellipsis_],
                           ellipsis_: ellipsis_},
    ...
    _TestCase(ChainMap[Type[Number], datetime.date], ZERO_OR_MORE,
              cli_repr=mapping_cli_repr_str(classpath_type_repr + '<:numbers.Number', date_repr),
              config_repr={classpath_type_repr + '<:numbers.Number': date_repr, ellipsis_: ellipsis_},
              test_val=collections.ChainMap({int: date_, complex: date_epoch}),
              cli_test_val=['{}={}'.format("int", date_.isoformat()),
                            '{}={}'.format("builtins.complex", date_epoch.isoformat())],
              config_test_val=[{"int": date_.isoformat(), "complex": date_epoch.isoformat()}]),
    _TestCase(Counter[types.FunctionType], ZERO_OR_MORE,
              ...
              cli_test_val=['{}={}'.format("int", 1), '{}={}'.format("len", 2), '{}={}'.format("__import__", -1234567890)],
              config_test_val={"int": 1, "len": 2, "__import__": -1234567890}),
    # parameterized collections
    _TestCase(List[int], ZERO_OR_MORE, int_coll_cli_repr, int_coll_config_repr,
              list_int, list_int_as_str, list_int),
    _TestCase(Set[int], ZERO_OR_MORE, int_coll_cli_repr, int_coll_config_repr,
              set_int, list_int_as_str, list_int, equal_contents),
    _TestCase(FrozenSet[int], ZERO_OR_MORE, int_coll_cli_repr, int_coll_config_repr,
              frozenset_int, list_int_as_str, list_int, equal_contents),
    _TestCase(Tuple[int, ...], ZERO_OR_MORE, int_coll_cli_repr, int_coll_config_repr,
              tuple_int_, list_int_as_str, list_int),
    # tuples
    _TestCase(Tuple[int, int, int], 3, tuple([type_spec(int)] * 3), [type_spec(int)] * 3,
              tuple_int_, list_int_as_str, list_int),
    ...
    _TestCase(LazyType["datetime.datetime"], None, type_spec(datetime.datetime), datetime_repr,
              datetime_, datetime_str, datetime_str),
    _TestCase(LazyListInt, ZERO_OR_MORE, "<List[int]>", [type_spec(int), ellipsis_],
              list_int, list_int_as_str, list_int),  # we registered with cli_nargs above to get '*' rather than None
    # custom classes
    _TestCase(custom_class, undefined, undefined, custom_class.config_repr,
              test_val=custom_class(),
              cli_test_val=undefined,
              config_test_val=custom_class().config_encode()),
    _TestCase(custom_class_with_varargs, undefined, undefined,
    ...
    _TestCase(custom_class_with_kwargs, undefined, undefined,
    ...
    _TestCase(custom_generic_class, undefined, undefined,
              config_repr=custom_generic_class.config_repr,
              test_val=custom_generic_class(1, 2, 3, xs={1: "two", 3: "four"}),
              cli_test_val=undefined,
              config_test_val=custom_generic_class(1, 2, 3, xs={1: "two", 3: "four"}).config_encode()),
    _TestCase(custom_generic_class[str], undefined, undefined,
              config_repr=custom_generic_class[str].config_repr,
              test_val=custom_generic_class("1", "2", "3", xs={1: "two", 3: "four"}),
              cli_test_val=undefined,
              config_test_val=custom_generic_class("1", "2", "3", xs={'1': "two", '3': "four"}).config_encode(str)),
    _TestCase(custom_generic_class[datetime.date], undefined, undefined,
              config_repr=custom_generic_class[datetime.date].config_repr,
              test_val=custom_generic_class(date_, xs={1: date_}),
              cli_test_val=undefined,
              config_test_val=custom_generic_class(date_, xs={'1': date_}).config_encode(datetime.date)),
]
